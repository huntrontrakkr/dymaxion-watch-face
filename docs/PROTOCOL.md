...
Byte 3 bit 6 hides the place icons beside the names in the time-zone drawer;
it is clear in older packets, so icons show. (The palette packet's glyph byte
is still sent but no longer read.) Byte 2 bit 1 turns the leading zero off (the first digit slot stays blank for
