...
import {calendarCells,usHoliday} from '../shared/calendar.js';
...
test('footer settings migrate, constrain panel order, preserve ranges and station choices',()=>{
  ...
  for(const patch of [{pages:[]},{pages:['zones','zones']},{rotationMinutes:3},{horizon:0},{shake:'yes'},{tide:{station:'evil/url'}}])assert.throws(()=>validateSettings({...defaults(),footer:{...defaults().footer,...patch}},zoneExists));
...
test('native panels validate actual provider packets and reject accidental shake patterns',()=>{
  ...
  for(const iso of ['2026-12-30','2027-01-01','2028-02-29','2026-03-08','2026-11-01','2027-06-19','2021-12-31'])for(const weekStart of [0,1])for(const weeks of ['current-next','previous-current']){
