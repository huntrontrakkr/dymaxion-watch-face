...
import {AXIS_GLYPHS,CHART,axisTextWidth,axisValue,chartLayout,chartX,chartY,chartHourLabels} from '../shared/chart-axis.js';
const font=JSON.parse(readFileSync('designer/public/type/proofs.json')).draft.text.small;
const maxHourWidth=clock24=>textWidth(font,clock24?'23':'12A');
test('measured chart gutters and dense hourly labels fit signed values, units and partial horizons',()=>{
  assert.equal(chartLayout('28','14',25).left,14);
  for(const [count,expected]of [[13,7],[25,9],[49,9]]){
    const layout=chartLayout('28','14',count),hours=Array.from({length:count},(_,i)=>(12+i)%24);
    assert.equal(chartHourLabels(layout,hours,true,font).length,expected);
  }
  ...
  for(const [lo,hi,decimal]of [[-150,280,false],[-35,155,true],[-10000,10000,true],[0,1000,false]])for(let count=2;count<=49;count++)for(const clock24 of [true,false])for(const range of [true,false])for(let start=0;start<24;start++){
    ...
    const labels=chartHourLabels(layout,Array.from({length:count},(_,i)=>(start+i)%24),clock24,font);
    if(range)assert(layout.left-3-Math.max(axisTextWidth(upper),axisTextWidth(lower))>=2);
  ...
  const hours=[0,1,1,2,3],labels=chartHourLabels(chartLayout('28','14',5),hours,true,font);
...
test('native and browser chart metrics, pixel glyphs and axis placements agree',()=>{
  ...
  for(const [lo,hi,decimal]of [[140,280,0],[-155,225,0],[-35,155,1],[-10000,10000,1],[0,1000,0]])for(let count=2;count<=49;count++)for(const clock24 of [0,1])for(const range of [0,1]){
    ...
    const widths=hours.map(hour=>textWidth(font,clock24?String(hour).padStart(2,'0'):`${hour%12||12}${hour<12?'A':'P'}`));
    cases.push([lo,hi,decimal,count,clock24,range,maxHourWidth(clock24),...hours,...widths].join(' '));
    const labels=chartHourLabels(layout,hours,clock24,font);
