...
import {galleryConfigs,GALLERY_SEED} from '../tools/gallery-configs.mjs';
...
test('gallery recipes are reproducible, importable, and cover every palette, clock and panel',()=>{
  ...
  assert.equal(values('id').size,64);
...
test('published gallery contains 64 distinct native-size frames with matching settings',()=>{
  const root=new URL('../designer/public/gallery/',import.meta.url);
  const manifest=JSON.parse(readFileSync(new URL('manifest.json',root),'utf8'));
  const hashes=new Set();
  assert.equal(manifest.seed,GALLERY_SEED);
  assert.equal(manifest.entries.length,64);
  ...
  assert.equal(hashes.size,64,'every preview must show a different face');
});
