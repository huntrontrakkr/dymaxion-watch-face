...
test('the watch places, draws and leads map times exactly as the workshop does',()=>{
  ...
  for(const set of SETS)for(const turn of [0,1])for(const [clock24,reserve] of [[1,0],[0,1]])for(const you of YOU)for(const size of [MAP_TIME_SMALL,MAP_TIME_MEDIUM,MAP_TIME_LARGE,MAP_TIME_XLARGE,MAP_TIME_WIDE,MAP_TIME_HUGE]){
...
test('labels sit in open ground, clear of each other, near their places',()=>{
  ...
  // taller ones may travel further (about 75, the wide ones about 95) to find room.
  const LIMIT=[280,400,400,400,500,600];
  for(const size of [MAP_TIME_SMALL,MAP_TIME_MEDIUM,MAP_TIME_LARGE,MAP_TIME_XLARGE,MAP_TIME_WIDE,MAP_TIME_HUGE])for(const set of SETS)for(const turn of [false,true]){
...
test('tiny figures: legible 3×5 to 5×10 sets; times keep the template width',()=>{
  for(const {height,glyphs} of TINY_FONTS){
    ...
    if(glyphs['0'][0].length===3)for(const c of TINY_CHARS)assert.equal(glyphs[c][0].length,TINY_FONTS[0].glyphs[c][0].length,'the narrow sizes share widths');
  }
  assert.deepEqual(TINY_FONTS.map(f=>f.height),[5,6,7,8,8,10]);
  assert.equal(tinyWidth('12:34'),17);
  for(let size=0;size<TINY_FONTS.length;size++)for(let h=0;h<24;h++)for(const clock24 of [true,false])for(const delta of [-1,0,1])
    assert(tinyWidth(mapTimeText({hour:h,minute:59,clock24,delta}),size)<=tinyWidth(mapTimeTemplate(clock24,true),size),'every time fits its template at every size');
...
test('map time size is the wearer\'s choice: every label uses it, stepping down only where it cannot fit',()=>{
  assert.deepEqual(MAP_TIME_SIZES,['small','medium','large','xlarge','wide','huge']);assert.deepEqual(TINY_FONTS.map(f=>f.height),[5,6,7,8,8,10]);
  let stepped=0,labels=0;
  for(const size of [MAP_TIME_SMALL,MAP_TIME_MEDIUM,MAP_TIME_LARGE,MAP_TIME_XLARGE,MAP_TIME_WIDE,MAP_TIME_HUGE])for(const set of SETS)for(const turn of [false,true]){
    const {places,obstacles,markers}=inputs(set,null,false,true);
    placeMapTimes(places,blocked,200,104,{obstacles,markers,size,turn}).forEach((s,i)=>{if(!places[i])return;assert(s,`${set[i]} finds a gap at size ${size}`);
      assert(s.size<=size);if(size<MAP_TIME_HUGE)assert.equal(s.size,size,'the smaller sizes always fit');
      if(size===MAP_TIME_HUGE){labels++;if(s.size<size)stepped++;}});
  }
  // The largest figures step down one size in crowded Europe and near the map's
  // edges (these sets are the hard cases: 12-hour times with a day offset).
  assert(stepped<=labels/2,`${stepped} of ${labels} labels stepped down`);
});
