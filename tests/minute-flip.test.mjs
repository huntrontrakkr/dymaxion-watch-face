...
test('native packed frames match the browser at every sampled stage of minute, hour and midnight transitions',()=>{
  ...
  execFileSync('cc',['-std=c11','-Wall','-Wextra','-Werror','-Iwatchface/src/c','tests/minute-flip-test.c','watchface/src/c/minute_flip.c','watchface/src/c/clock_styles.c','-o','test-results/minute-flip-test']);
...
test('every numeral style shares the transition: native frames match the browser for Span and each font',()=>{
  mkdirSync('test-results',{recursive:true});
  execFileSync('cc',['-std=c11','-Wall','-Wextra','-Werror','-Iwatchface/src/c','tests/minute-flip-test.c','watchface/src/c/minute_flip.c','watchface/src/c/clock_styles.c','-o','test-results/minute-flip-test']);
  const timings=[0,80,160,240,320,400];
  const styles=[['span',0],['leco',5],['bitham-bold',6],['bitham-light',7],['bitham-medium',8],['leco-delta',9]];
