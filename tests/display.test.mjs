...
test('display preferences migrate, validate, and travel separately from the stable layout packet',()=>{
  const s=defaults();assert.deepEqual([...encodeDisplay(s)],[1,4,1,0],'new faces use Chamfer figures');
  assert.deepEqual([...encodeDisplay({...s,clockDisplay:'broad'})],[1,2,1,0]);
  for(const clockDisplay of ['span','triangles','broad','chamfer'])assert.notEqual(encodeDisplay({...s,clockDisplay})[1],3,'code 3 is the retired LCD style');
