...
test('display preferences migrate, validate, and travel separately from the stable layout packet',()=>{
  const s=defaults();assert.deepEqual([...encodeDisplay(s)],[3,4,36,0,0,22,7,10],'new faces use Chamfer figures and conditional map times');
  assert.deepEqual([...encodeDisplay({...s,clockDisplay:'broad'})],[3,2,36,0,0,22,7,10]);
  ...
  assert.deepEqual([...encodeDisplay({...defaults(),clockDisplay:'span',leadingZero:false})],[3,0,38,0,0,22,7,10]);
  assert.equal(defaults().mapBackground,'none','the map background is off by default');
  assert.deepEqual([...encodeDisplay({...defaults(),mapBackground:'points'})],[3,4,36,1,0,22,7,10]);
  assert.deepEqual([...encodeDisplay({...defaults(),mapBackground:'lines'})],[3,4,36,2,0,22,7,10]);
  assert.deepEqual([...encodeDisplay({...defaults(),mapBackground:'fine-points'})],[3,4,36,3,0,22,7,10]);
  assert.equal(defaults().zoneTimes,'when-hidden','place times appear elsewhere only when absent from the panel');
  assert.equal(defaults().mapTimesTurn,false,'map times stay horizontal unless turning is allowed');
  assert.deepEqual([...encodeDisplay({...defaults(),zoneTimes:'when-hidden'})],[3,4,36,0,0,22,7,10]);
  assert.deepEqual([...encodeDisplay({...defaults(),zoneTimes:'always',leadingZero:false})],[3,4,42,0,0,22,7,10]);
  assert.deepEqual([...encodeDisplay({...defaults(),zonePosition:'map',mapTimesTurn:true})],[3,4,100,0,0,22,7,10],'bit 6: map times may turn');
  assert.equal(defaults().zonePosition,'map','conditional place times appear on the map by default');
  ...
  assert.equal(defaults().placeIcons,true,'place icons are on by default, whatever the theme');
  assert.equal(encodeDisplay(defaults())[3]&64,0);assert.equal(encodeDisplay({...defaults(),placeIcons:false})[3]&64,64,'bit 6 hides them');
  assert.equal(encodeDisplay({...defaults(),zoneTimesTall:true})[3]&32,32,'bit 5 tall place times');
  assert.throws(()=>validateSettings({...s,mapTimeSize:'giant'},zoneExists));
  ...
  const legacy={...s};delete legacy.zoneTimes;delete legacy.zonePosition;
  assert.equal(validateSettings(legacy,zoneExists).zoneTimes,'panel','older files keep place times in the panel');
  assert.equal(validateSettings(legacy,zoneExists).zonePosition,'left','older files retain their original position fallback');
  const saved=validateSettings({...s,zoneTimes:'always',zonePosition:'right'},zoneExists);
  assert.equal(saved.zoneTimes,'always');assert.equal(saved.zonePosition,'right','explicit preferences survive the new default');
