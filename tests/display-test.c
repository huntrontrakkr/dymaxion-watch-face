...
int main(void){
  ...
  p[0]=2;assert(!display_valid(p,4));p[0]=1;p[1]=2;assert(display_valid(p,4));p[1]=3;assert(display_valid(p,4));p[1]=4;assert(display_valid(p,4));p[1]=5;assert(display_valid(p,4));p[1]=8;assert(display_valid(p,4));p[1]=9;assert(display_valid(p,4));p[1]=10;assert(!display_valid(p,4));p[1]=1;p[2]=2;assert(display_valid(p,4));p[2]=3;assert(display_valid(p,4));p[2]=4;assert(!display_valid(p,4));p[2]=1;p[3]=1;assert(display_valid(p,4));p[3]=3;assert(!display_valid(p,4));p[3]=64;assert(!display_valid(p,4));p[3]=4;assert(!display_valid(p,4));
