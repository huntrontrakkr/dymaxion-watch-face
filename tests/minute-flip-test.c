...
// minute-flip-test <broad | geodesic-resource.bin> <from> <to> <ms>...
int main(int argc,char **argv){
  ...
  const ClockFace *face=&BROAD_FACE;ClockFace geodesic;static uint8_t resource[65536];
  if(strcmp(argv[1],"broad")){
    ...
    assert(geodesic_face_init(&geodesic,resource,length));face=&geodesic;
    assert(!geodesic_face_init(&geodesic,resource,length-1));assert(geodesic_face_init(&geodesic,resource,length));
  }
