...
const masks={points:triangleGridMask(m,'points',rows),lines:triangleGridMask(m,'lines',rows),'fine-points':triangleGridMask(m,'fine-points',rows)};
...
test('both backgrounds span the strip edge to edge, from the top of the net to its bottom',()=>{
  assert.deepEqual(MAP_BACKGROUNDS,['none','points','lines','fine-points']);assert.deepEqual(BACKGROUND_BITS,{none:0,points:8,lines:16,'fine-points':32});
...
test('the baked map flags each background only where the net leaves the strip empty',()=>{
  for(let i=0;i<bytes.length/4;i++){
    ...
    if(!empty)assert.equal(flags&(BACKGROUND_BITS.points|BACKGROUND_BITS.lines|BACKGROUND_BITS['fine-points']),0);
  }
