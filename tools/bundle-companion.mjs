...
const mobile=await build({entryPoints:['tools/mobile-config.js'],bundle:true,write:false,format:'iife',target:'es2017',minify:true,loader:{'.bin':'base64'}});
const html=readFileSync('tools/mobile-config.html','utf8').replace('__STYLE__',()=>readFileSync('tools/mobile-config.css','utf8')).replace('__SEARCH_STYLE__',()=>readFileSync('shared/place-search.css','utf8')).replace('__PALETTE_STYLE__',()=>readFileSync('shared/palette-controls.css','utf8')).replace('__SCRIPT__',()=>mobile.outputFiles[0].text);
