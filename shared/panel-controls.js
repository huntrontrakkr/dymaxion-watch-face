...
export function panelControls(root,getSettings,onChange){
  ...
  root.innerHTML=toggle('enabled','Enable bottom panels','A 44-pixel band replaces the place clocks one panel at a time.')
    ...
    +toggle('weather.humidityLine','Humidity on the weather chart','A dotted line on a fixed 0–100% scale; the header gives the value.')+toggle('weather.tideMarks','High and low tides on the weather chart','Small triangles on the baseline when a tide station is set.')+toggle('weather.rangeLabels','Show range labels')+toggle('weather.grid','Show a faint midline')+`</details></details>`
