...
// (clear by default); bit 0 is retired. Byte 3 indexes MAP_BACKGROUNDS.
export function encodeDisplay(settings){
  const style=DISPLAY_CODES[settings.clockDisplay],background=MAP_BACKGROUNDS.indexOf(settings.mapBackground??'none');
  if(style===undefined)throw new Error('Invalid clock display.');
  if(background<0)throw new Error('Invalid map background.');
  return new Uint8Array([2,style,settings.leadingZero===false?2:0,background]);
}
