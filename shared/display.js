...
export const MAP_TIME_SIZE_CODES=['medium','small','large'];
...
// MAP_BACKGROUNDS, bits 2-3 the map time size (0 medium 3×6, 1 small 3×5,
// 2 large 3×7; older packets read as medium). Bytes 4-6 are power and motion (shared/power.js). The watch
...
export function encodeDisplay(settings){
  ...
  return new Uint8Array([3,style,(settings.leadingZero===false?2:0)|zones<<2|position<<4|(settings.mapTimesTurn?64:0)|(settings.nameplate?128:0),background|MAP_TIME_SIZE_CODES.indexOf(settings.mapTimeSize??'medium')<<2,...encodePower(settings.power)]);
}
